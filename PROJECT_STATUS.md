...
| `ELEVENLABS_API_KEY` / `ELEVENLABS_VOICE_ID` | Higher-quality Bangla TTS for the phone confirmation call (`eleven_v3` model — the only ElevenLabs model with Bengali support) | Falls back to Twilio's native `bn-IN` `<Say>` (lower quality but free) | **Not configured** — see open issue below, blocked by ElevenLabs billing, not code |
...
4. **Phone confirmation call is now in Bangla** (`bn-IN`) — see `src/app/api/twilio/voice/[orderId]/route.ts` and `.../gather/[orderId]/route.ts`. `quickIntent` in the gather route now matches Bangla confirm/decline phrases too (হ্যাঁ/না/বাতিল/কনফার্ম etc.) before falling back to Gemini.
5. **ElevenLabs Bangla TTS is built but inactive.** `src/lib/tts.ts` + `src/app/api/tts/[id]/route.ts` call ElevenLabs' `eleven_v3` model (the only one with Bengali) and `speak()` in `src/lib/twilio-webhook.ts` prefers it via `<Play>`, falling back to Twilio's native `bn-IN <Say>` automatically if unconfigured or on error. Live-tested with a real key: **ElevenLabs' free plan blocks ALL non-self-cloned voices from API access** (both the default "Rachel" voice and a Voice Library pick 402'd with `paid_plan_required`), and Instant Voice Cloning is itself a paid-tier-only feature — so there's currently no free path to a working `ELEVENLABS_VOICE_ID`. Code is confirmed working otherwise; just needs a paid ElevenLabs plan (~$6/mo Starter unlocks Voice Library API access) and a voice id pasted into `.env` to activate.
